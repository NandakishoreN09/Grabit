...
import { MdOutlineReplay, MdOutlineTimer } from "react-icons/md";
...
export default function AdminOrders(): JSX.Element {
    ...
    const fetchOrders = async () => {
      try {
        ...
        const fetchedOrders = querySnapshot.docs.map((doc) => {
          ...
          return { id: doc.id, ...order, timeLeft: remainingTime };
        });
  ...
  // ✅ Function to format items correctly
    ...
    try {
      ...
      setOrders((prevOrders) =>
        prevOrders.map((order) =>
          order.id === orderId ? { ...order, status: newStatus, timeLeft: newStatus === "Ready for Pickup" ? 0 : order.timeLeft } : order
        )
      );
    } catch (error) {
  ...
  // ✅ Add Extra 10 Minutes in Firestore
  const addExtraTime = async (orderId: string) => {
    try {
      const orderRef = doc(firestore, "orders", orderId);
      setOrders((prevOrders) =>
        prevOrders.map((order) =>
          order.id === orderId && order.status === "Preparing"
            ? { ...order, timeLeft: (order.timeLeft || 30) + 10 }
            : order
        )
      );
      await updateDoc(orderRef, { timeLeft: (orders.find(order => order.id === orderId)?.timeLeft || 30) + 10 });
    } catch (error) {
            ...
            >
              ...
              <div className="flex flex-col items-center">
                <div className="flex items-center gap-2">
                  ...
                  {/* Add Extra 10 mins button */}
                  {order.status === "Preparing" && (
                    <motion.button
                      onClick={() => addExtraTime(order.id)}
                      className="bg-orange-500 p-1 rounded-full text-white hover:scale-110 transition"
                      whileHover={{ scale: 1.2 }}
                      whileTap={{ scale: 0.9 }}
                      title="Add 10 mins"
                    >
